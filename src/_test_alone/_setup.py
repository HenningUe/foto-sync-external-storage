...
class InitTasks(object):
    ...
    def adapt_constants():
        global SRC_PATH
        import fotosync
        fotosync.ProgressHandler._wait_between_progress_update = 0.1
        from datasync import _glob
...
class FolderStructCreator(object):
    ...
    def create_ini_file():
        global TRG_PATH
        from datasync import FotoSyncIni
        tmpl = """
        [{computer_name}]
        ...
        with codecs.open(FotoSyncIni.get_config_file_path(), "w", encoding="UTF-8") as f:
            f.write(tmpl)
