...
class DirectoryGetter(object):
    ...
    @classmethod
    def get_trg_dir(cls):
        check_ini_file_ok = FotoSyncIni.check_ini_file()
        if check_ini_file_ok is False:
            return None

        trgt_dir = FotoSyncIni.read_target_dir()
...
def _get_my_drive_name():
    if _glob.my_drive_name is None:
        my_drive = _get_my_drive()
        drive_name = winapi.get_volume_name(my_drive)
        return drive_name
    else:
