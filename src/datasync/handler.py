...
class DataSyncHandler(object):
    ...
    def sync_files_specific(self, last_changes):
        ...
        copied_files = list()
        if not any(last_changes):
            return copied_files
        already_copied_image_list = self._file_copier_tagger.restore_image_list()
        already_copied_image_list = set(already_copied_image_list)

        files_to_be_copied = list()
        for last_chg in last_changes:
            if not last_chg['action'] in ['Created'] \
               or not last_chg['file_dir_type'] in ['file'] \
            ...
            if src_dir.lower() in self.data_sync_config.directory_blocklist \
               ...
               or src_rel_path in already_copied_image_list:
                continue
    ...
    def sync_files_full(self):
        ...
        src_files_all = {os.path.join(x[0], y) for x in os.walk(self.src_dir_root) for y in x[2]
                         if not (os.path.basename(x[0]) == self._file_copier_tagger._META_DATA_DIR_NAME_DEFAULT)}
        ...
        for src_rel_file in src_rel_files_all:
            if src_rel_file in already_copied_image_list \
    ...
    def _copy_files_to_be_copied(self, files_to_be_copied):
        ...
        for i_file, file_to_be_copied in enumerate(files_to_be_copied):
            ...
            already_copied_image_list.add(src_rel_path)
            copied_files.append(src_rel_path)
        if any(copied_files):
            self._file_copier_tagger.store_image_list(list(already_copied_image_list))
        return copied_files
...
class FileCopierTagger(object):
    ...
    def store_image_list(self, image_list):
        if self._store_meta_at_src:
            self._store_image_list(self._get_src_data_storage_dir(), image_list)
        self._store_image_list(self._get_trg_data_storage_dir(), image_list)

    def restore_image_list(self):
        if self._store_meta_at_src:
            storage_dir = self._get_src_data_storage_dir()
            image_list = self._restore_image_list(storage_dir)
            if not self._store_meta_at_trg \
               or (image_list is not None and any(image_list)):
                return image_list
    ...
    def _restore_image_list(self, storage_dir):
    ...
    def _store_image_list(self, storage_dir, image_list):
        existing_json_files = self._get_existing_json_backup_files(storage_dir)
        ...
        data = dict(image_list=image_list, timestamp=timestamp)
    ...
    @staticmethod
    def _serialize_to_json(json_target_file, image_list):
        if not os.path.isdir(os.path.dirname(json_target_file)):
            storage_dir = os.path.dirname(json_target_file)
            os.makedirs(storage_dir)
            winapi.set_file_attr_to_hidden(storage_dir)
        json_raw = json.dumps(image_list)
